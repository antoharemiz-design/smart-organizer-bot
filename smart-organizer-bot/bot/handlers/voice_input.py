...
# Инициализация Whisper (ленивая загрузка)
_model = None
...
async def handle_voice(message: Message, calendar_service: LocalCalendarService):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api.telegram.org/file/bot{message.bot.token}/{file_path}") as resp:
        ...
        # Обрабатываем как обычный текст через NLU
        ...
        if nlu_result and nlu_result.intent == "task" and nlu_result.task_draft:
            ...
            await message.answer(
                "😕 Не удалось разобрать задачу. Возможно, текст содержит нестандартные формулировки.\n"
                "Попробуйте сказать по-другому или напишите текстом."
            )
